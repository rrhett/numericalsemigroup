...
##
#Y Copyright (C) 2003, Rhett S. Robinson, Clemson University
##
...
DeclareOperation ("NSGSChain", [IsNumericalSemigroup]) ;
DeclareOperation ("NSGTypeChain", [IsNumericalSemigroup]) ;
...
# These compare B to L, either as a chain or a specific B[i] and L[i].
# They also take one more argument as to whether to print the results or not
DeclareOperation ("NSGICompareBL", [IsNumericalSemigroup, IsBool]);
DeclareOperation ("NSGICompareBL", [IsNumericalSemigroup, IsInt, IsBool]);
